...
extension UserDefaults {
    private enum Key: String {
        case sensorID = "com.libre2client.sensor.sensorID"
    ...
    public var sensorID: String? {
        get {
            if let astr = string(forKey: Key.sensorID.rawValue) {
        ...
        set {
            if let newValue = newValue {
                set(newValue, forKey: Key.sensorID.rawValue)
            } else {
                removeObject(forKey: Key.sensorID.rawValue)
            }
        }
    }
